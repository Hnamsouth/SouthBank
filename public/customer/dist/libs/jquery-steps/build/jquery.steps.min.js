    ...
    function D(a, b, c) {
        if (b.enablePagination) {
            var d = a.find(".actions a[href$='#finish']").parent()
                ...
                n = a.find(".actions button[href$='#finish']").parent()
            ;
            ...
            //   e._enableAria(c.stepCount > 1 && c.stepCount > c.currentIndex + 1)) : (n._showAria(b.enableFinishButton && c.stepCount === c.currentIndex + 1),
            b.enableFinishButton && b.showFinishButtonAlways ? (n._enableAria(c.stepCount > 0),
                e._enableAria(c.stepCount > 1 && c.stepCount > c.currentIndex + 1)) : (n._showAria(false),
    ...
    function L(a, b, c) {
        if (b.enablePagination) {
            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>'
                ...
                , e2 = '<li><button type="submit" class="" href="#{0}" role="menuitem">{1}</button></li>'
                , f = "";
            b.forceMoveForward || (f += e.format("previous", b.labels.previous)),
                f += e.format("next", b.labels.next),
            b.enableFinishButton && (f += e2.format("finish", b.labels.finish)),
